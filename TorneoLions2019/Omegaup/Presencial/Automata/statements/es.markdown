# Descripción

Al hacer una investigación acerca de la automata célular, te has encontrado con el juego de la vida hecho por el matemático íngles John Conway en 1970. Este juego no es algo que tu "juegas", sino que es una simulación de pequeñas "células" o pixeles que viven, mueren y reproducen. Al ver el juego en acción, te darás cuenta que estas células tienden a interactuar con otras células cercanas para hacer figuras y animaciones extrañas, y despues de muchos turnos estas figuras tienden a desaparecerse o se repiten formando patrones.

![Juego de la vida][1]

El juego consiste en un arreglo bidimensional de células y en cada turno se debe calcular el siguiente estado del juego sin modificar al estado pasado.

Curioso de saber más, te pones a leer las reglas:

Cada célula depende de las **8** células a su alrededor:
 - Una célula viva con menos de dos vecinos se muere debido a la falta de población.
 - Una célula viva con dos o tres vecinos sigue viva en el siguiente turno.
 - Una célula viva con más de tres vecinos se muere debido a la sobrepoblación.
 - Una célula muerta con exactamente tres vecinos vivos se hace viva debido a la reproducción.

Al leer más acerca de este juego, te das cuenta que existen ciertas configuraciones iniciales que son caóticas o inestables; es decir, si despues de **N** turnos no se repite ningun estado o turno del juego, esa configuración es inestable. Has decidido crear un programa para identificar si una configuración inicial del juego de la vida es inestable o no.

Además, en caso de llegar a una configuración que se repite o que se desaparece (todas las células se mueren), quisieras saber después de cuantos turnos pasa esto.

# Entrada

Recibirás un número **N**, el número de turnos hasta donde se debe simular. Por ejemplo, si este valor es 5, se debe simular desde el estado 0 (la inicial) hasta el estado 5.

Luego, recibiras **8** líneas con **8** carácteres representando la configuración inicial que deseas probar. Es decir, el tamaño del juego será de **8** x **8** o de 64 espacios.

El carácter **#** representará siempre una célula viva y el carácter **.** siempre representará una célula muerta.

# Salida

 - En caso de ser inestable:

Se debe imprimir el mapa con la última configuración utilizando los mismos carácteres de la entrada (# y .) en **8** líneas distintas, luego se debe imprimir la palabra **inestable** en otra línea.

 - En caso de que se desaparecen o se mueren todas las células:

Se debe imprimir el mapa vacío (8 x 8 puntos) y se debe imprimir en otra línea **desaparece despues de K turnos** donde **K** es el número del turno en el que desaparecen las ultimas células.

 - En caso de que se repiten unos turnos:

Se debe imprimir el mapa con el primer estado que se repite y se debe de imprimir en otra línea **repite despues de K turnos** donde **K** es el número del primer turno que se vuelve a repitir después.

# Ejemplos

Ejemplo #1: La simulación termina después de 1 turno y se determina que la configuración es inestable debido a que no se repitieron turnos.

||input
1
........
........
..###...
....#...
...#....
........
........
........
||output
........
...#....
...##...
..#.#...
........
........
........
........
inestable
||end

Ejemplo #2: La configuración se repite en el turno 0, 2, 4, 6, etc... además del 1, 3, 5, 7, etc... Se escoge 0 porque es el primer turno que se repite más adelante.

||input
200
........
........
..###...
........
........
........
........
........
||output
........
........
..###...
........
........
........
........
........
repite despues de 0 turnos
||end

Ejemplo #3: La figura expande y desaparece en el turno 25.

||input
50
........
........
........
....##..
...##...
....#...
........
........
||output
........
........
........
........
........
........
........
........
desaparece despues de 25 turnos
||end

Ejemplo #4: La figura expande y no se repite ningun estado en los turnos de 0 a 10.

||input
10
........
........
........
........
........
.#......
...#....
##..###.
||output
........
........
........
..##....
.##.....
...#....
........
........
inestable
||end

Ejemplo #5: La figura expande y llega a una configuración estable, pero como el límite de turnos es 11 no se llega a repitir esta figura porque no se simula el siguiente turno. Si **N** fuera 12, se imprimiría que se repite en el turno 11.

||input
11
...#....
..###...
.#####..
........
........
........
........
........
||output
........
........
...#....
..#.#...
..#.#...
...#....
........
........
inestable
||end

# Límites

$1 \le K \le N \le 200$


  [1]: data:image/gif;base64,